...
int main(int argc, char** argv) {
	...
	if (is_icp)
	{
		...
		icp.setMaxCorrespondenceDistance(resolution * 1.1);
		icp.setTransformationEpsilon(1e-11);
		icp.setEuclideanFitnessEpsilon(resolution * 1.1);
		icp.setMaximumIterations(1000);
		icp.setUseReciprocalCorrespondences(true);
		icp.align(*reg_cloud_S, gror.getFinalTransformation());
		std::ofstream file(tfm_path.c_str());
		file << std::fixed << std::setprecision(10) << icp.getFinalTransformation() << std::endl;
